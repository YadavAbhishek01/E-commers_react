...
const Category = () => {
  return (
    <div className='main-container  overflow-x-hidden w-full px-25 py-4 '>
        <h1 className='font-bold text-2xl'>Categaries</h1>
      <div id='categary' className='container flex mb-3 gap-4   overflow-x-auto  '>
       {product.map((item)=>(
      
        <div key={item.id}  className="flex  p-3   flex-col shrink-0  rounded-md shadow-md w-[300px]  ">
            <Link href={`/list?cat=${item.category.trim()}`} >
